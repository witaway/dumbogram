...
public class PagingQueryParser<TEntity> where TEntity : BaseEntity
{
    ...
    public PagingQueryParser(IPagingQueryKeysetParsingStrategy<TEntity> getKeysetStrategy, int takeLimit = int.MaxValue)
    {
    ...
    private static PaginationDirection GetDirection(PagingQuery pagingQuery)
    {
        ...
        if (isFirstRequested) return PaginationDirection.Forward;

        var isLastRequested = pagingQuery.Last;
        if (isLastRequested) return PaginationDirection.Backward;

        var isNextRequested = pagingQuery.NextPageToken is not null;
        if (isNextRequested) return PaginationDirection.Forward;

        var isPreviousRequested = pagingQuery.PrevPageToken is not null;
        if (isPreviousRequested) return PaginationDirection.Backward;
    ...
    private Cursor<TEntity> GetCursor(PagingQuery pagingQuery)
    {
        ...
        if (pagingPointNotSpecified) pagingQuery.First = true;
