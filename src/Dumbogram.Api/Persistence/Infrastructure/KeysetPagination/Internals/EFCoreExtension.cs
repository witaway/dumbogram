...
public static class EFCoreExtension
{
    ...
    ) where TEntity : BaseEntity
    {
        ...
        foreach (var column in columns.Skip(1)) orderedQuery = column.ApplyThenOrderBy(orderedQuery, sortDirection);
    ...
    ) where TEntity : BaseEntity
    {
        ...
        if (keyset.Columns.Count == 0) throw new Exception();

        if (take == 0) throw new Exception();
        ...
        if (result.Any())
        {
            ...
            if (cursor.Type == CursorType.Last || direction == PaginationDirection.Backward) pagedResult.Reverse();
