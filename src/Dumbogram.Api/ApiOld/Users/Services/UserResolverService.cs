...
using Dumbogram.Api.ApiOld.Users.Services.Errors;
using Dumbogram.Api.ApiOld.Users.Services.Exceptions;
...
namespace Dumbogram.Api.ApiOld.Users.Services;
...
public class UserResolverService
{
    ...
    public async Task<Result<UserProfile>> TryGetApplicationUser()
    {
        ...
        if (claimsPrincipal == null) return Result.Fail(new UnauthorizedError());

        var applicationUserIdResult = claimsPrincipal.TryGetApplicationUserId();
        if (applicationUserIdResult.IsFailed) return Result.Fail(applicationUserIdResult.Errors);
        ...
        if (applicationUserResult.IsFailed) return Result.Fail(applicationUserResult.Errors);
    ...
    public async Task<Result<ApplicationIdentityUser>> TryGetIdentityUser()
    {
        ...
        if (identityUserIdResult.IsFailed) return Result.Fail(identityUserIdResult.Errors);
        ...
        if (identityUserResult.IsFailed) return Result.Fail(identityUserResult.Errors);
    ...
    public async Task<UserProfile> GetApplicationUser()
    {
        ...
        if (applicationUserResult.IsFailed) throw new UnauthorizedException();
