...
public class MessagesService
{
    ...
    public async Task<IEnumerable<Message>> ReadManyMessages(Chat chat)
    {
        var query = _dbContext
            .Messages
            .Where(message => message.Chat == chat);
    ...
    public async Task<Result<IEnumerable<Message>>> QueryManyMessages(UserProfile subjectUser, Chat chat)
    {
        var messagesCanBeReadResult = await _messageActionsGuardService.CheckMessagesCanBeRead(subjectUser, chat);
        if (messagesCanBeReadResult.IsFailed) return Result.Fail(messagesCanBeReadResult.Errors);
